...
export default async function ChatbotPage() {
  ...
  const [{ data: profile }, { data: qaList }] = await Promise.all([
    ...
    supabase.from("chatbot_qa").select("*").eq("profile_id", user.id).order("position", { ascending: true }),
  ]);
  if (!profile) redirect("/onboarding");

  return <ChatbotManager initialProfile={profile} initialQaList={qaList || []} />;
}
